...
class AppraisalValuationFingerprint
{
    public function for(Appraisal $appraisal): string
    {
        ...
        return hash('sha256', implode('|', [
            ...
            $this->normalize((string) ($vehicle?->make ?? '')),
            $this->normalize((string) ($vehicle?->model ?? '')),
            $this->normalize((string) ($vehicle?->variant ?? '')),
            (string) ($vehicle?->year ?? ''),
            $this->normalize((string) ($vehicle?->transmission ?? '')),
            $this->normalize((string) ($vehicle?->fuel_type ?? '')),
            (string) ($vehicle?->mileage ?? ''),
            $this->normalize((string) ($vehicle?->city ?? '')),
